...
import { Tabs, Card, Statistic, Row, Col, Tag } from 'antd';
...
const historyData = [
  ...
  { day: 'T3', maxLevel: 1.60 },
  { day: 'T4', maxLevel: 1.65 },
  { day: 'T5', maxLevel: 1.70 }, // Đỉnh cao nhất
  { day: 'T6', maxLevel: 1.62 },
  { day: 'T7', maxLevel: 1.58 },
  { day: 'CN', maxLevel: 1.50 },
];
...
const CurrentView = () => (
  <div className="space-y-6">
      ...
      <Col span={8}>
        <Card bordered={false} className="bg-slate-50">
          <Statistic
            title="Cảnh báo"
            value="Mức 2"
            prefix={<Tag color="warning" className="mr-2">Báo động</Tag>}
    ...
    <div className="h-[300px] w-full">
      <h4 className="text-slate-600 font-semibold mb-4">Biểu đồ mực nước theo giờ (Thực đo)</h4>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={currentData}>
          ...
          <YAxis domain={[0, 2]} />
          <Tooltip />
          <ReferenceLine y={1.5} label="Báo động I" stroke="orange" strokeDasharray="3 3" />
          <ReferenceLine y={1.6} label="Báo động II" stroke="red" strokeDasharray="3 3" />
          <Area type="monotone" dataKey="level" stroke="#0891b2" fillOpacity={1} fill="url(#colorLevel)" name="Mực nước (m)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  </div>
...
const ForecastView = () => (
  <div className="space-y-4">
    <div className="h-[350px] w-full">
      <h4 className="text-slate-600 font-semibold mb-4">Dự báo mực nước 24h tới</h4>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={forecastData}>
          ...
          <Line type="monotone" dataKey="level" stroke="#0891b2" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 8 }} name="Dự báo mực nước (m)" />
        </LineChart>
      </ResponsiveContainer>
    </div>
    <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200 text-yellow-800 text-sm">
      <strong>Lưu ý:</strong> Đỉnh triều cao nhất dự kiến đạt 1.65m vào lúc 20:00 tối nay. Các khu vực trũng thấp ven sông cần đề phòng ngập úng.
    </div>
  </div>
);
...
const HistoryView = () => (
  <div className="space-y-4">
    <div className="h-[350px] w-full">
      <h4 className="text-slate-600 font-semibold mb-4">Đỉnh triều cao nhất 7 ngày qua</h4>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={historyData}>
          ...
          <Tooltip />
          <Legend />
          <Bar dataKey="maxLevel" fill="#0891b2" radius={[4, 4, 0, 0]} name="Đỉnh triều (m)">
            {historyData.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={entry.maxLevel >= 1.6 ? '#ef4444' : '#0891b2'} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
