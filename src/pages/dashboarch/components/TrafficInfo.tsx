...
import { Car, MapPin, AlertTriangle, Clock } from 'lucide-react';
...
const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'congestion') return <Tag color="error" icon={<AlertTriangle size={12} />}>Kẹt xe</Tag>;
  if (status === 'slow') return <Tag color="warning" icon={<Clock size={12} />}>Chậm</Tag>;
...
const RoutesView = () => (
  <div className="space-y-4">
    <Row gutter={16} className="mb-4">
      ...
      <Col span={8}>
        <Card bordered={false} className="bg-orange-50">
          <Statistic
            title="Điểm kẹt xe"
            value={floodedRoutes.filter(r => r.status === 'congestion').length}
    ...
    <h4 className="text-slate-600 font-semibold">Danh sách các tuyến đường ảnh hưởng</h4>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {floodedRoutes.map(route => (
        <Card key={route.id} size="small" className="shadow-sm hover:shadow-md transition-shadow border-slate-200">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h5 className="font-bold text-slate-800 text-base">{route.name}</h5>
          ...
          <div className="flex items-center gap-4 text-sm text-slate-600 my-2">
            <div className="flex items-center gap-1">
              ...
              Độ sâu: <span className="font-semibold text-blue-600">{route.depth}cm</span>
            </div>
            <div className="flex items-center gap-1">
